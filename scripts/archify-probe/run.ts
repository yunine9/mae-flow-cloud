...
const semantic = `只返回 JSON，不要代码围栏。根据提供的设计材料提炼模块协作图，不补造不存在的功能。不是生产现状审计。格式：{"title":"标题","modules":[{"id":"英文短ID","name":"模块名","summary":"20字内职责摘要","responsibility":"完整职责","interfaces":"提供或消费的接口/材料；未明确则如实写","acceptance":"设计要求的验收行为，不声称已验证","evidence":"原文中一段逐字引用"}],"relations":[{"from":"模块ID","to":"模块ID","label":"12字内关系说明"}]}。覆盖主要参与者、文档会话、发布存储、检视与子任务协作；合理合并，不超过9个模块。关系表示调用或材料流转，不代表任务必须串行。只表达设计内容，不生成坐标、布局或Archify字段。`;
