...
if(existsSync(root)) throw Error('Preview directory already exists; refusing to overwrite it');
...
const auth=new LocalAuth(join(root,'auth.json'));auth.bootstrapAdmin('admin','mae-flow-demo');auth.createUser('dev','mae-flow-demo','developer','本地体验');
const data=JSON.parse(readFileSync(join(out,'fast-1.json'),'utf8'));
...
for(const m of data.modules){
 ...
 source.cards=[{dot:'cyan',title:m.name+' · 具体工作',items:[m.responsibility,m.interfaces]}];
 diagrams.push({id:m.id,view:'development',source});
}
writeFileSync(join(artifactDir,'story.md'),story);
