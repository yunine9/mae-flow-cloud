...
- **内核唯一权威**:流程规则、门禁契约、证据判定只在 ../mae-flow
  ...
  唯一例外(用户 2026-08-24 拍板):task_type=issue 的问题处理流程
  不走内核——流程规则由 assets/skills-dts 技能+issueFlowPrompt 承载,
  收口/催办/交付判定在 taskService 的 issue 分叉里。判定逻辑仍然不许
  复刻内核的;只是这一类任务根本没有内核流程可复刻。
