...
export async function runDomainKnowledge(input: DomainExecution, options: DomainAgentOptions) {
  ...
  const tools = [extractionSkillTool(skill), sourceTool, codeSearchTool(input.evidence), documentTool, knowledgeMaterialTool(materials, join(options.dataDir, "knowledge-materials"), observe), changesTool,
    wxdoubaoTool(signal, observe), knowledgeEvidenceTool(() => input.job.evidence), ...(research ? [research.tool(), workers!.tool()] : [])];
  ...
  try {
    ...
    let outcome = await session.start((research ? `本次是持续业务知识研究任务，补全读代码无法得知的背景、业务意图、规则原因、隐含约束和历史经验。业务资料与无线豆包是主要来源，源码仅按具体疑问辅助核对，不要求扫描或读取每个仓。先用 knowledge_research 保存知识问题和资料线索，持续补证；多项独立调查可用 knowledge_delegate 分工。完成项引用业务资料实际返回的 evidence_id，不能用源码代替业务意图依据。普通回复不会结束研究；知识主题调查和最终资料核对完成后才结束。既有草稿可完善，人工修改受保护。\n${research.anchor()}\n\n` : "") + extractionSkillMission(skill, {
