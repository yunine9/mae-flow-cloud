...
export function researchDocumentTool(input: ResearchExecution) {
  ...
  return defineTool({
    ...
    description: "分段保存同一篇 Markdown 的组件清单、跨仓关系和正文。outline 只登记已确认对外提供的细粒度能力，不能把 public 可见性当作对外依据，不能将仓库直接当组件；每项默认纳入专家审核。read 不带 id 返回清单，带 id 返回完整组件。section 必须带对外接口及依据、集成产物/依赖、含代码块的最佳示例和来源。讨论只读，返工只修改指定项，不能改其他项或整体说明。",
...
export function jointResearchMission(input: ResearchExecution): string {
  ...
  const discipline = [
    ...
    "先通过 research_document outline 分批登记已确认对外的能力清单（可随研究补充）；同一能力跨仓实现时合为一项并注明各仓职责，不把同名 API 混为同一语义。每个配置仓都需实际读取，核对对外发布的接口及构建目标是否有对应能力，而非让每个内部模块都成为一项。说明跨仓依赖、组合用法、初始化和销毁顺序及版本差异。",
