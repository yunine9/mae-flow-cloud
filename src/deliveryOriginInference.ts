...
/** Product reporting convention: generic follow-up repairs count as review;
 * explicit pipeline clues take priority; all other follow-up commits count as review. */
export function inferCommitOrigin(message: string): { origin: CodeOrigin; evidence: string[]; explicitRepair: boolean } | undefined {
  ...
  const origin: CodeOrigin = pipeline ? "pipeline" : "review";
  return { origin, explicitRepair: pipeline || review, evidence: [`推断：提交说明包含${pipeline ? "流水线／质量检查修复" : review ? "检视反馈" : "未发现明确流水线修复线索，按统计口径归检视修改"}线索（非平台记录确认）`] };
}
