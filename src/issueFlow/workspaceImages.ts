/**
 * 会话工作区图片回显(#376):分析报告与过程文档里嵌入的截图引用
 * (`ticket-images/<单号>/<哈希16>.<扩展名>` 工单截图、
 * `issue-images/<哈希16>.<扩展名>` 登记截图)经此从会话工作区读盘
 * 返给浏览器。拉单时工单截图已落工作区(ticketImages.ts),历史会话
 * 不在回收重货名单、图一直在盘上——读落盘件而不是回 DTS 实时取,
...
const FILE_SEGMENT = "([0-9a-f]{16})\\.([a-z]+)";

/** 校验前端回传的 path 形态。合法返回工作区相对路径原文(键与落盘
 * 相对路径同串);非法/缺席返回 undefined。单号段带 `..`(穿越形态)
 * 一并拒——与 ticketImages 落盘时的单号校验同尺。 */
