...
export function issueFixedOpeningPrompt(
  ...
  return [
    ...
    "6. 问题修复按 TDD 走:先写(或改)能复现问题的单测,再改码让它转绿,"
      ...
      + "流水线全绿)才放行进换库验证——有红当场打回带失败项:修复后同"
      + "分支再推、重建 MR、重新申报;在跑则受理,你可停等,绿了平台自动"
      + "放行;全绿未申报平台只会开回合提醒你申报,不会替你推进。",
    ...
    "9. 持续维护 issue-analysis.md(结论/证据链/置信度/下一步建议四要素),"
      + "它是本会话的核心交付物:结论首行=一句话根因,证据链用出处指针不贴"
      + "日志原文,submit_analysis 交的是收敛后的结论版(四章节齐全是门票)。",
    ...(options.moonlight
...
export function issueOpeningPrompt(
  ...
  return [
    ...
    "7. 持续维护分析报告 issue-analysis.md(结论/证据链/置信度/下一步建议"
      + "四要素),它是本会话的核心交付物:结论首行=一句话根因,证据链用出处"
      + "指针不贴日志原文,提交的是收敛后的结论版(四章节齐全是门票)。",
    "",
