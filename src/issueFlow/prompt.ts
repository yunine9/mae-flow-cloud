...
典型顺序:拿单→拉日志→分析→对齐→建分支→实施→提交→换库→验证→推送→提MR。但:

- **非问题是合法且常见的终点**:研究后判定误报/需求误解/无法复现,把证据和结论写进 issue-analysis.md,上报阶段 concluded,向用户说明后收口归档即可,**不要**硬走编码交付。
- 用户可只要其中某一步(如"只定位一下");任一步发现新情况(日志暴露另一个根因),回到「对齐」重新对齐。
- 某步不适用(无需换库/无日志环境)就跳过。
- 每进入新环节,调用一次 \`report_stage\`(stage + 一句话 note)——平台靠它显示你正在干什么。
`;
...
研究结论是"非问题"(误报/环境问题/需求误解/无法复现)时:在 issue-analysis.md 写清证据与结论 → \`report_stage\` stage=concluded → 向用户说明,建议归档。**不要**为了"走完流程"而制造代码修改。
`;
...
- 提 MR 后上报 stage=submitting_mr;会话收口由用户归档。
`;
