...
- 每个改过的仓:各自分支(同名)→ 各自 push_branch → 各自 create_mr;建齐后调 `complete_stage` **申报 MR 清单**(mrs 参数,MR 链接或仓地址)——平台验绿:清单=台账(少报/多报点名打回)且流水线全绿才放行;有红当场打回带失败项,修完同分支再推、重建 MR 后重新申报;流水线在跑则受理,可停等,绿了自动放行。UT 结果(`report_ut`)只是事实上报,不是建 MR 的前置。
