...
{{lead}}已发起推送确认卡(带本次变更摘要),git push 未执行。请结束本回合等用户确认——确认后平台会通知你重新推送本分支;确认一次只放行一个仓的一次推送,多仓交付逐仓走「举卡→等作答→推本仓→再举下一仓」,等待期间不要再调 push_branch(含其他仓,只会再被拦);若用户答「暂不推送」,按其意见调整后再重新发起确认

## push.review.force

{{lead}}已发起强制覆盖确认卡(本次推送将覆盖远端同名分支——同单重跑场景,卡面带远端旧分支指向与变更摘要),git push 未执行。请结束本回合等用户确认——确认后平台会通知你重新推送本分支(重推不必再带 force 参数,确认过的覆盖语义随令牌走);确认一次只放行一个仓的一次推送,多仓交付逐仓走「举卡→等作答→推本仓→再举下一仓」,等待期间不要再调 push_branch;若用户答「暂不推送」,按其意见调整后再重新发起确认(该分支已有 MR 时,覆盖后原 MR 随之更新,不要重复创建)

## push.busy

一次只推一个仓:已有 push_branch 在途,等它返回后再推下一个仓。多仓交付是串行节奏——逐仓「推送→(过目确认)→下一仓」,不要在同一回合并发调用多个 push_branch

## push.gate_pending

推送确认卡已在等用户作答:先结束本回合。确认一次只放行一个仓的一次推送——这张卡作答后先推卡上那个仓,收到作答通知再逐仓发起下一个仓的推送;等待期间继续调 push_branch 只会被再次打回
...
MR 清单核验通过(没有改动、无需 MR),「提交 MR·跑绿」阶段已收口。收口不等于流程完成——还差环境验证这一步:请立即调 raise_gate 工具(kind=env_verify)把环境验证卡交给用户,然后结束本回合等待用户作答,不要自行继续。

## mrgate.all_green

MR 核验通过({{repos}}):全部 MR 流水线跑绿,「提交 MR·跑绿」阶段已收口。收口不等于流程完成——还差环境验证这一步:请立即调 raise_gate 工具(kind=env_verify)把环境验证卡交给用户,然后结束本回合等待用户作答,不要自行继续。
