...
> 2026-09-11 勘误：同仓单元已经由平台强制串行，允许同一责任人共用
> 同一 AR 单号。原“重复必须拦截”的判断已撤销；输入框仍保留逐单元
> 编辑能力，用于业务上确实需要不同 AR 的情况。
...
## 第五轮续测：额度恢复后的全链收口（2026-09-01）

本节承接上文的真实现场，不新建剧本替代原任务。额度恢复后继续使用真实
GLM `glm-5.1`、Linux Build-Fix、浏览器最终检视和浏览器假平台合入，直至
task-1 的三个串行单元、task-6 的两个并行仓子任务都进入 `completed`。此前
“额度使后半链不能宣称通过”的结论仍是当时真实状态；本节给出恢复后的最终
结果。

### MFC-055 · P1 · 子任务把主方案已确认的仓库事实再次问人（已修）

- **实测失败现场**：隔离子任务工作区看不到兄弟仓源码时，Agent 把 CHAIN
  中已确认的仓名、互链路径与术语又当成未知事实提问，正常 owner 会怀疑前面
  的跨仓方案没有传下来。
- **根因**：子任务使命只让 Agent “去读 CHAIN”，没有把单元任务书中的仓名、
  负责面和已确认事项声明成权威输入；隔离工作区的合理缺失被误判为需求缺口。
- **修复**：子任务使命明确以单元任务书和已确认事项为准；只有当前仓代码与
  契约冲突或出现真正的新业务决定才允许举卡，不再要求访问兄弟仓来重复确认。

### MFC-056 · P1 · 目标分支前进后，交付 diff 与负责面门禁混入别人的改动（已修）

- **实测失败现场**：串行/并行任务合入使目标分支前进后，最终检视的文件数与
  增删统计会包含目标分支新出现、但本任务没有贡献的文件；同一批文件还可能被
  负责面门禁误判为本单元越界。
- **根因**：交付贡献仍从任务最初基线计算；它适合看整单累计变化，却不是
  “当前目标分支到本任务 HEAD”的贡献集合。
- **修复**：当目标分支是 HEAD 祖先时，以 `target..HEAD` 计算本任务贡献；
  只有祖先关系不成立时才保守回退原基线。文件树、统计、白名单和越界门禁
  共用同一贡献事实；空白行也按 Git 的 `+/-` 如实计数。

### MFC-057 · P0 · 最终检视选择“需要调整”后仍处于不可编辑阶段（已修）

- **实测失败现场**：task-8 在最终检视要求修正术语后，页面看似把意见交给
  Agent，内核却仍停在 `external_verify`；Agent 被告知该阶段不能修改，旧 SHA
  可能原样再次进入交付，流程既卡人又可能假闭环。
- **根因**：宿主只改了等待卡/交付 loop，没有通过内核干预合同把阶段退回
  `delivery_review`，也没有作废旧 SHA 的 Build-Fix、流水线和推送证据。
- **修复与真实复验**：统一 `reopenKernelForPushReview()` 执行内核阶段回退并
  清理所有 SHA 绑定证据。task-8 随后真实修改文档，生成新 SHA
  `1d7ad7549174`；页面重新展示新 diff，经人审、Build-Fix、流水线和 MR 合入
  后完成。这个阶段不是产品限制，原行为就是流程路由 bug。

### MFC-058 · P1 · “重跑续推”误唤醒普通编码 Agent（已修）

- **实测失败现场**：Build-Fix 容器消失或 MR 创建失败后点击“重跑续推”，
  除了续跑宿主交付动作，还会额外启动一轮普通 Agent；用户会看到与当前失败
  无关的回复，并浪费模型额度。
- **根因**：通用重试把所有 `failed/verifying + stalled` 归到任务会话恢复，
  没有区分 Build-Fix、平台动作与真正的编码停机。
- **修复**：Build-Fix 失败只重跑 Build-Fix；外部 MR/流水线等待只恢复宿主
  交付链；仅真实处于外部验证等待点时才走平台重试。新增回归钉死“已有检视
  loop 不唤醒 Agent”。

### MFC-059 · P0 · 假平台忽略 repo，多仓第二仓永远建不了 MR（已修）

- **实测失败现场**：task-8 已推送 `origin-aux.git`，创建 MR 却返回 HTTP 400；
  假平台拿辅助仓分支名去默认主仓查，自然报分支不存在。此前“多仓回归”即使
  前面跑通，也无法完成第二仓的真实 MR 链。
- **根因**：MR、流水线、门禁与合入的协议虽携带 `repo`，假平台实现全都只
  使用启动参数中的单一 bare 仓。
- **修复与真实复验**：MR、流水线状态、三门禁和浏览器合入均按 repo 路由，
  且仅允许访问默认 bare 仓同目录下已登记的本地裸仓；同 SHA 的流水线也按仓
  隔离。重启修复版平台后，task-8 通过“重跑续推”创建辅助仓 MR !1，三门禁
  全绿并在浏览器合入，task-6 随即自动完成。

### MFC-060 · P2 · 返工新增文件沿用旧白名单，容易漏交付（未修，交互方案待定）

- **实测现场**：task-7 首轮最终检视只保留 1 个文件；返工后 Agent 新增
  `docs/notification-troubleshooting.md`，新文件已提交但未自动进入交付范围，
  必须由人再次手工勾选。
- **根因**：最终白名单被当成跨返工轮持久意图；系统无法判断“以前排除”与
  “本轮新出现”应否自动纳入。
- **风险与建议**：当前行为不会偷偷推文件，但会漏交付。下一轮应把白名单按
  review HEAD 版本化：旧文件保留人的选择，新 SHA 首次出现的文件默认纳入并
  明确标“本轮新增”，由人再排除。

### MFC-061 · P1 · 跨仓并行任务的后发契约修正不会自动传播（未修，需方案）

- **实测现场**：服务仓在检视阶段修正统一术语后，已经并行执行的 SDK 子任务
  不会收到新契约；最终由 owner 在 SDK 检视中再次指出并修正才保持一致。
- **根因**：CHAIN 是拆单时快照，子任务之间只有依赖/完成状态，没有“契约
  修订版本 + 受影响单元重验”的传播合同。
- **风险与建议**：这不是一句 prompt 能可靠补齐。建议由主任务维护契约版本，
  影响共享契约的返工先举主任务变更卡，再机械标记相关子任务需要重读/重验。

### MFC-062 · P2 · 专注审阅弹层没有把后台内容从可访问树隔离（未修）

- **实测现场**：全屏专注审阅打开后，视觉上后台不可点，但自动化/辅助技术仍
  可定位后台的最终决定单选项；一次中心点击被前景 diff 截获并打开了逐行批注。
- **根因**：弹层有固定定位和高 z-index，但后台没有 `inert`/等价焦点隔离。
- **边界**：普通鼠标用户不会看见后台按钮，本轮不在没有焦点方案的情况下
  夹带修改；应按无障碍弹层标准统一处理焦点圈、Escape 与后台 inert。

### MFC-063 · P1 · 构建容器消失会停机，但诊断与恢复链通过（外部原因未定位）

- **实测现场**：task-4 首次提交 `973e06a` 后，Build-Fix 运行中的容器消失，
  任务停机并生成诊断包。点击“重跑续推”后未丢代码，重新完成领域归档，最终
  形成 `c75b4ab` 并交付。
- **结论**：容器为何被外部删除没有足够证据，不能冒充已定位；但产品的停机
  解释、诊断包、代码保留与恢复路径均经真实现场验证通过。

### 最终逐项实测结论

#### 场景一：单仓拆三块

1. **实测通过**：分析先查仓库，再分批澄清真正的业务语义，每题有建议答案；
   重问已确认仓库事实的问题按 MFC-055 修复。
2. **实测通过**：划分方向固定卡出现，选择“你看着切”后正常生成方案。
3. **实测通过**：首单元是接口声明、空实现与接线骨架，整体编译通过；全局
   注册和构建依赖归契约单元。原 scope 缺口由 MFC-052 的确认门禁兜住。
4. **实测通过**：CHAIN 有“已确认事项清单”且无 TBD；新 scope 满足路径段
   闭合、唯一归属及任务书提及路径覆盖。
5. **实测通过**：不填各单元单号时被可读拒绝；填写
   `REQ2026090101/02/03` 后原子确认成功。
6. **实测通过（完整链）**：task-3 在 task-2 合入前等待，task-4 在 task-3
   合入前等待；前一 MR 浏览器合入后下一单元自动解锁。三个子任务最终均
   `completed`，父 task-1 自动显示“全部 3 个子任务已完成”。
7. **实测通过**：每个任务正文均一页内写清第 N/3、负责面、上下游和单号；
   每个工作区均有 `chain-plan.md`。

#### 场景二：负责面门禁与越界裁决

1. **实测通过**：task-4 推送前点名单元及三个越界文件并停摆，未绕过门禁。
2. **实测通过**：主任务 owner 收到现场，工作台显示越界裁决卡。
3. **实测通过（真浏览器）**：以 `other-dev` 登录并实际点击“放行”，HTTP 403，
   页面明确要求联系 `main-owner`，状态与文件清单不变。
4. **实测通过**：放行后同批文件可在 MR 检视并续推；task-7 的打回只用新增
   提交 `209cb4a` 撤出点名文件，面内实现保留。`git log` 确认没有 reset/rebase；
   后续目标分支前进两次也都通过合并目标分支、新 SHA、重新验证恢复。
5. **实测通过**：负责面 `src/filter/` 时，`src/filterX/other.ts` 被判越界。

#### 场景三：回归

1. **实测通过（完整链）**：多仓不勾单仓拆分项，分析后 task-7/task-8 并行；
   分别保留 `REQ2026090120/21`、各自仓知识与负责面。主仓 MR !8、辅助仓
   MR !1 均浏览器合入，父 task-6 自动完成。跨仓后发术语传播缺口见 MFC-061。
2. **实测通过（完整链）**：单仓不勾选项的 task-5 没有分析阶段，直接编码、
   Build-Fix、最终检视、三门禁、MR !3 合入并 completed。
3. **实测通过**：Chain 节点显示“仓名 · 单元名”与负责面；父卡展示 N/M 已
   合入；同仓子任务在列表按单元名、序号和状态可直接区分。

### 完整性与现场留存

- task-2/3/4/5/7/8 及父 task-1/task-6 的账本最终均为 `completed`。
- 任务克隆与源仓 `.git/objects` 均未发现硬链接；源仓仅保留 `master`，没有
  多出任务分支，源仓隔离通过。
- `.pilot/e2e-unit-split-glm-20260901/` 继续按 label 原样保留，没有删除或混入
  产品提交。
