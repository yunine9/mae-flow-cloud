...
唯一消息 ID、插件能否携带固定 Header、HTTP 响应如何显示给用户。若插件
...
业务仓自带的 Skill 不再全量自动灌入。发起任务时在「02 交付定位」下的
「业务知识与 Skill（可选）」点击「读取知识与 Skill」，页面按仓同时列出
项目规则、`docs` 业务文档和 Skill；用户可把容易遗漏的文档选为本单重点
知识，也可只勾本单需要开放给 Agent 的能力。Skill 支持四个固定位置：
...
```
<仓库>/AGENTS.override.md              # 有它时优先于 AGENTS.md
<仓库>/AGENTS.md                       # Pi 自动加载，不需要勾选
<仓库>/CLAUDE.md                       # 兼容内部既有仓，Pi 自动加载
<仓库>/docs/**/*.{md,mdx}              # 页面可选的本单重点知识
```

规则文件只展示“自动加载”，不制造一个无意义的复选框。`docs` 最多选择
12 篇、合计 256 KiB；AGENTS 明确提到的文档会标为“建议本单加载”。选中
文档在会话开局进入上下文，Agent 后续 Read/Grep、Skill 正文读取也会写入
`knowledge-events.jsonl`。工作台「执行现场 → 本单知识足迹」展示消费资源、
次数、会话身份和内核阶段，便于复盘哪些知识真正提效、哪些经常漏选、哪些
Skill 需要补说明。该台账是 fail-open 观测旁路，不能替代质量证据或卡住流程。
