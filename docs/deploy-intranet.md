...
4. **最小启动(界面优先形态,用户拍板"参数该在界面配")**:
   ...
   **`--kernel-mode` 必须与 `--platform <网关地址>` 同时给**(本地演练可
   用 `--fake-platform`):云端把编译/UT/CodeCheck 交给权威流水线，流程
   会停在 `external_verify` 等平台事实，没有平台就没人核销，每一单都会
   卡在"验证中"。少给这个参数服务会**直接拒绝启动**并说明原因，不会
...
「仓内能力（可选）」点击「读取仓内能力」，页面按仓列出名称、description
和相对路径；用户只勾本单需要开放给 Agent 的能力。支持四个固定位置：
