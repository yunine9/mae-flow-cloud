...
通知中的任务链接不得使用 `127.0.0.1`。正式部署在启动参数或配置文件
中设置 `--public-url http://<内网域名或内网IP>:8787`；未设置时平台会
从已登录用户实际访问的 Host（含反代的 `X-Forwarded-Host/Proto`）生成。
`--public-url` 优先级最高，适合多网卡或反向代理部署。
