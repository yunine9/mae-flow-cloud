...
页面不应把每轮返工画成重新走完整流程。高层进度保持稳定，而且**流程环节不减**
（2026-09-02 用户拍板：流程没变，只是内核在交付段之后扩展了持续检视能力，前台
尽量不感知）——阶段词表只有内核 `flow/phases.json` 一份，原来的“交付”改叫
“检视与验证”，末尾加一段终态“已合入”：
...
进入交付闭环后，高亮始终停在“检视与验证”。它不是一个很快经过的尾部步骤，而是承载多轮反馈、验证和等待合入的稳定阶段；页面只在下面切换本轮活动：
...
- ~~不设置默认修复轮数上限~~ **已修订(2026-09-01 用户拍板)**:保留
  ...
  CI 不再用同交付 SHA 推断放弃；按配置预算收敛，需要人工时通过 AskUserQuestion 明确举卡；
...
10. `continuous_review` 的启用事实另存为按真实仓库路径绑定的工作区外宿主文件，
    ...
    terminal 验签保留完整生命周期。反馈开批、结果登记、重放与索引恢复统一只核验
    `delivery_loop` 中的反馈事实，不要求工作步骤、流水线或介入记录与旧收据一致；
    批次、回复、暂缓决定和发布记录仍需有宿主收据，不能通过新反馈把被改写的旧意见洗成可信事实。
    新的反馈收据不替代 `pipeline-record` 或 `close` 的质量与合入凭据。FeedbackStore 对“最后一行是完整 JSON、但语义非法且没有换行”的情况
