...
## 低上下文纪律(内网模型窗口只有约 17 万 token,撑爆过一次)

上下文是消耗品,别拿它装原始材料。四条硬规矩:

1. **一阶段一会话**。做完一个阶段就开新会话,阶段之间靠**文件交接**:
   把该阶段的结论、路径、已确认的字段形状写进 `~/pilot-notes/stage-N.md`
   (每条一行,不抄大段原文),新会话开场只读这一个文件 + 本手册对应
   那一节。不要把上一阶段的全部对话背进新会话;
2. **搜,不要读**。既有框架源码仓一律用 `grep -rn "<关键字>"` 定位,
   命中后只看那一处上下几十行(`sed -n '起,止p'`),**禁止整目录通读、
   禁止 `cat` 大文件**。本仓文档同理,按需取节:
   - adapter.json 参考填法 = `sed -n '346,441p' docs/mr-loop-adaptation.md`
   - 端点契约 = `sed -n '80,165p' docs/mr-loop-adaptation.md`
   - 部署手册整篇 34KB,只查你当下要的小节,别整篇读;
3. **长输出先落文件再取尾巴**:`npm test > /tmp/t.log 2>&1; tail -40 /tmp/t.log`。
   报错原文进报告时也只摘关键几行 + 说明,不要整段粘贴;
4. **报告边做边追加到文件**(`>>` 到 `~/pilot-report.md`),不要攒在
   上下文里最后一次性生成——撑爆一次就全丢了。

真撑爆了:开新会话,读 `~/pilot-notes/` 里的交接文件继续,不要从头再来。
