...
首次创建 MR 前，由责任人填写对应 AR 单的准确描述。卡片简要说明：合入时
要求 MR 标题与 AR 描述一致，平台目前无法读取描述，需要从 AR 单复制。
标题原样使用该描述，不拼接单号、任务名或编译跳过后缀；关联单号仍通过
`dts_no` 单独传给平台，编译跳过事实保留在交付台账和工作台。

答复保存在 `waiting.json`，按任务与 AR 单号绑定；同一 AR 的重试、修复和
重启复用已填内容，换 AR 则重新询问。已有 MR 不补问、不擅自改标题。
月光模式不代填；该卡由 Cloud 接续交付，不额外启动 Agent 会话。
验证见 `tests/mrDescription.test.ts`：真实 Git/内核与假平台联通、权限、
标题及 AR 关联、重启恢复；内网平台最终合入检查仍需部署后验收。

## 2026-09-10：人工合入的持续监听

MR 创建成功即落盘并启动监听，不等待流水线绿灯。已有 MR 的交付续跑、
服务重启后的修复/排队/暂停/停摆任务也恢复监听；已完成、已取消任务除外。
默认每 10 秒查询一次（沿用 `poll_interval_s`），网络查询耗时会影响实际发现时间。
修复期间只消费远端合入事实，不额外派一轮修复。

平台报告已合入后，停止 Agent、Build-Fix 和容器，由可信宿主登记完成；
保留旧流水线失败及原验证 SHA，实际合入 SHA 单独记录。并发查询共用一次
停止和收口；取消或原位重跑后，迟到的旧查询不能改写任务。

验证：`tests/mrLifecycleWatch.test.ts` 覆盖恢复、停止、并发和取消，包含
真实 Git/Python 内核、假平台的首轮失败后合入用例；内网 CodeHub 和真实
构建容器的联调仍需部署后验收。下文是最初适配分析，现状以本节和代码为准。
