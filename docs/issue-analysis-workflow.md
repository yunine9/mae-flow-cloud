...
落工作区根目录,`submit_analysis` 机械校验四章节齐全,缺一章打回。写法三原则:**结论先行**(首行=一句话根因,只读第一行即可拍板)、**证据指针化**(一句话+出处指针,日志/代码原文不贴,可复核靠指针可走)、**提交前收敛**(交的是结论版,过程探索压掉,报告是决策材料不是过程回放):

| 章节 | 内容要求 |
|---|---|
| `## 结论` | 首行=一句话根因(≤50字);随后多可疑点按可能性排序;非问题写判定依据 |
| `## 证据链` | 表格;每条=一句话证据+出处指针:代码 File:行号、日志(时间戳+目录+grep 关键字,关键行≤2行)、截图文件;验证结果(已确认/已排除/待确认) |
| `## 置信度` | 高/中/低 + 为什么不是更高(≤2行);分析基线与问题版本分支的差异写这里 |
| `## 下一步建议` | 按仓一行一条:修复方向/待确认清单/加日志验证方案;非问题写归档建议 |
| `## 现象` | 放最后,≤5行:现象/影响/发生条件/版本分支;正向流程图等参考资料挂节末 |
