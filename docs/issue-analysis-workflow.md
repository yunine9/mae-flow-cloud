...
落工作区根目录,`submit_analysis` 机械校验四章节齐全,缺一章打回。首行是一句话总结(串联 现象→根因→方案,只读这一行即可拍板)。写法三原则:**节名即问题**(现象/根因/方案各自成节,短小直答)、**证据指针化**(结论随文标出处指针——代码 File:行号、日志关键行一行,日志/代码原文不贴,可复核靠指针可走)、**提交前收敛**(交的是结论版,过程探索压掉,报告是决策材料不是过程回放):

| 章节 | 内容要求 |
|---|---|
| (首行) | 一句话总结,串联 现象→根因→方案(≤60字) |
| `## 问题现象` | ≤5行:现象/影响范围/发生条件/版本分支;正向流程图等参考资料挂节末 |
| `## 问题根因` | 写定论,结论随文标出处指针;未收敛时如实列可疑点(按可能性)+缺什么条件,不硬编排序;非问题写判定依据 |
| `## 修改方案` | fix 阶段照此执行的承诺,不是建议:按仓一行一条,改哪里/怎么改/怎么验证(加日志验证方案写这里);非问题写"无需修改"或归档建议 |
| `## 置信度` | 高/中/低 + 为什么不是更高(缺环境、无法复现等证据局限也写这里,≤2行);分析基线与问题版本分支的差异写这里 |
...
| 机制 | 位置 | 作用 |
|---|---|---|
| 四章节门票 | `submit_analysis`(tools.ts) | 报告结构完整从提示词约定升级为工具层关卡 |
...
- **当前口径:人工评审起步**——分析报告(四章节齐全)即评审载体,业务/测试同学评审定位结论;
...
> 本仓 docs/ 目录维护业务知识(领域事实与排障方法,按目录+文件名组织);需要业务事实时优先查阅,引用其中内容时把文件路径随文标注进报告。
