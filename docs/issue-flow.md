...
- **对象只有分析报告**:代码层面的意见经右栏插话/续聊直接告知 AI
  ...
  再检视。
- **运行中也能交办**：攒批后点击「提交 N 条修改意见」，按本次草稿快照发送。
  正在运行则经 steer 在当前工具结束后读取；空闲则继续当前会话；等人时完整保存，随当前问题答复送达。
  上一批尚未完成时仍能追加，新草稿不会自动混进已经提交的批次。
- 提交不再默认回退分析、增加轮次或清空 UT/流水线/MR 登记，不撤销待回答的问题卡。
  Agent 按实际影响修改并验证；旧提交的验证结果不能冒充新代码通过。
  人工接管、终态和转正继承报告的原有权限边界保留。
- 报告版本快照和 `reviews.jsonl` 原文仍保留；协作流展示「批量交办」及接收说明。
  已接收、已读取和处理完成是不同事实，完成后仍需核对修订内容。
- **闭环刻意从简**:不做逐条回执与逐条裁决——锚点四态检测(hit/
  moved/gone/ambiguous)只服务草稿(ADR-0025「新版干净纸面」:sent
  意见锚在自己批次的冻结版上,冻结文本永不漂移),白送「已被改动·
  请你确认」与「原文仍在」徽标,修订后的新版报告 + 分析确认卡上的
  整体把关收口;
