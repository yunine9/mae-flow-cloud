...
给空态,不拖垮会话。发言入口收拢在右栏 NEXT ACTION(六态互斥,含收口
后的续聊);对话的**复盘阅读面**在材料页签的过程文档子视图里(过程
问答),现场的「消息」筛选管原始事件,三者各司其职。

**材料页签**(`src/issueFlow/materials.ts`,路由 `/issues/:id/materials*`):

- DTS 单据卡片:复用 DTS 列表页签的字段对拍与图片代理,会话绑了单号即可展开。
- **过程文档**(`src/issueFlow/documents.ts`,路由 `/issues/:id/documents*`
  与 `/issues/:id/dialogue`;2026-08-29 起取代原"结论文档"单文件视图):
  多页签形态对齐任务工作台的过程文档(ws-tabs)——
  - **分析报告**:`issue-analysis.md`,固定首页,即写即读,AI 续写后
    随状态自动重读;文件名是行为契约(prompt/闸门票/转正继承都认它),
    页签名才叫"分析报告"。
  - **过程问答**:从 events.jsonl 投影的复盘对话,口径见 ADR-0008——
    只呈现问答卡、用户决策(卡答与闸答;平台闸的问句快照随
    human_decision 事件落账,投影据此合成"问"的半边)、用户主动
    输入与检视意见;agent 的过程性发言不进。只追加、不回写;上限
    500 回合,触顶如实标注。
  - **动态文档**:Agent 落在会话根目录的其他 .md,每份一页签,最近
    修改在前;只扫顶层(repo/、skills/、local-logs/ 不算过程文档)。
...
- **提交 = 整体回退的人工触发源**:攒批一次「提交 N 条意见并重跑
  ...
  提交即:挂起的 Agent 问题卡 supersede 作废(留审计原因)→ 平台闸
  随 `fixedRollback` 清除 → 意见标记送出、被检视报告留版本快照
  (`reviews/issue-analysis@*.md`,子目录不进过程文档清单)→ 整体
  回退问题分析(round+1、其后阶段标 redo、UT/流水线/MR 申报账作废、
  分支与 MR 延用)→ 意见清单注入新一轮分析,落 `review_submitted`
  事件(过程问答里"这轮为什么重跑"看它)。
- **闭环刻意从简**:不做逐条回执与逐条裁决——锚点四态检测(hit/
  moved/gone/ambiguous)白送「已被改动·请你确认」与「原文仍在」
  徽标,修订后的新版报告 + 分析确认卡上的整体把关收口;
  `submit_analysis` 重新举卡时清 `review_active`,检视入口恢复。
- 运行中可以圈草稿但不能提交(等 AI 停机或举卡);运行中的回合
  不被检视打断。
