...
## 会话执行现场(六一级页签,#123 拍平后的现状)

会话主栏七个一级标签:**元信息 / 对话现场 / DTS单据 / 分析报告 / 工作区变更 /
拉取日志 / 逐仓交付**(#123 把原"材料"面板的二级页签升格拍平,#260 起
"过程文档"页签改名"分析报告"并收敛为单报告;下文的
"材料"多为历史口径,现指这组页签)。数据全部旁路 fail-open,任何一块
失败给空态,不拖垮会话。发言入口收拢在右栏协作区(六态互斥,含
收口后的续聊);对话的**复盘直播**在对话现场页签(协作流)。
