...
- 登记:「问题处理」页手工登记(有单填单号走七阶段,无单留空走三节点;
  ...
  ——描述里的内嵌图由宿主带同源 token 经 `/issues/dts-file` 代理回取,
  浏览器不直连内网域。固定流程在此页签补填仓/模块/网管环境(拉单
  页签同款字段)。(当前一次一张,批量只留了 UI 口子)
...
- **版本多选过滤**:拉取后把可发起单的版本汇总去重,按 R 版降序、
  R 同比 C 版降序(如 `V100R025C10` → R=25/C=10,解析不出的垫底);
  下拉多选,**默认勾选并列最高 R/C 的全部版本**,之后勾选/取消由
  用户接管。
