...
- 流水线红灯处置(2026-09-13 起 #247/ADR-0024 修订,票 03/81/82 的
  平台分诊与代举已退场):红灯终态处理只发送失败事实(失败摘要、逐维度
  明细含工具名、镜像产物落点 pipeline/ 目录、上轮报错对比、三路处置
  指引)给 AI,或修或举卡由它现场判断——发送回合=修复回合(reds
  发送时 +1,绿了清零),同提交刹车与预算耗尽停机语义不变,
  ...
  **证据回灌闸**(`pipeline_evidence`):AI 判断报错原文有缺口、无法
  ...
  证据注入回灌回合(reds 不重复计数——发送回合已记同一轮),AI 按
  原文修复后同分支再推、重新监看。
  两张卡问的都是人工事实,月光永不代答。
...
- 登记有两个独立入口。**手工登记只用于无单问题**:标题、现象描述、
  ...
  保文字,#276);日志等**登记附件**(2026-09-19,类型不限、单个
  500MB)点选/粘贴/拖拽上传,落暂存后把 `attachments/<hash>.<ext>`
  相对路径以纯文本插进描述,登记提交时复制进会话工作区——附件是给
  AI 读的分析材料,开场词单列一行引导优先查看,前端不预览;描述框打开即
  ...
  完整单号可自动远程补查;「介入档位」列可在发起前按单定档(四选:
  跟随全局/全自动/优先报告/优先对齐,偏好存本地按人,发起成功即清),
  随创建请求定格进会话,发起后不可改(ADR-0057)。单号直达 DTS 门户
  ——描述里的内嵌图由宿主带同源 token 经
...
- **登记指派(2026-09-16,ADR-0031)**:手工登记第五项必填「责任人」——
  ...
  介入档位(全局档;会话级特例随会话走,ADR-0057)、同单去重全随
  责任人走;登记人(通常是测试)对会话零操作
...
- **元信息页签与用户指派仓(2026-09-11,ADR-0023)**:会话工作台首签
  ...
  提交到 `POST /issues/:id/repos`,平台只校验+留痕+经平台回合通道发送
...
request_env)全程开放;调查取证(pull_repo)与阶段性
...
### 分支推送(直推;过目闸 2026-09-17 退役)

`push_branch` 是分支唯一出门口(容器里 git push 被禁用),平台只做
机械校验(单号门禁、分支名 `master_<工号>_<单号>`、在途互斥——同毫秒
并发调用一个放行一个打回,issue-14 收口)后直推。曾经存在的推送前过目
闸(ADR-0009,push_confirm 平台闸 + 一次性确认令牌)已在 2026-09-17
随介入档位收敛整体退役(ADR-0009 增补):所有档位直推,闭环速度优先;
需求交付侧的个人设置「推送前过目」是另一套实现,原样保留。

**强制覆盖(2026-09-11 增补,ADR-0009)**:同单重跑撞远端遗留同名分
支时,`push_branch` 带 `force: true` 即覆盖重推。覆盖一律走租赁式核对
(`--force-with-lease`,以推送前探测的远端 tip 为期望值,远端又被推进
即拒推,不盲盖);non-fast-forward 拒收回执指路 force=true,含"已有
MR 随覆盖更新、勿重复创建"的提醒。
...
- **版本多选过滤(按 B 版前的版本段分组,2026-08-29;2026-09-13
  ...
  由用户接管。过滤入口 2026-09-24 起住工具栏(「版本」标签 + 漏斗
  弹层,壳同单号/标题的列头过滤;同日版本列与分支列一并退役,ADR-0038
  修订)——版本组内任一单未命中分支映射标红,展示中的单存在未映射时
  过滤框旁出「前往配置」警示条;过滤与工具栏搜索 AND 叠加,且只作用
  于名下列表(远程补查单豁免)。
...
- **上下文自动整理**：主会话、子 Agent 和预热/修复会话共用 CloudSession 的
  请求前容量策略。系统提示词、工具定义、当前消息和临时知识一起计入预算，
  通常约七成容量就整理旧历史；预留输出较大时更早整理。分析报告路径和修改方案
  仍随确认推进消息注入，修复阶段摘要也带报告指针。旧的
  `issue-compact-every-events` / `issue_compact_every_events` 参数仅兼容读取，
  不再触发压缩；阶段切换也不再强制多调用一次模型。原会话与压缩边界使用 Pi
  原生 checkpoint 保存，取消或摘要失败不替换历史。详见
  [上下文容量策略与验收](session-context-budget-2026-09-21.md)。
