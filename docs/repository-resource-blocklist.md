...
配置复用管理员 API：`PUT /settings/execution-policy`，字段 `blocked_repository_resources` 为字符串数组。省略字段保留当前规则，空数组清空。配置保存在平台数据目录的 `settings.json`，不写入业务仓。

下单页（需求和问题登记）展示一行屏蔽提示。点击可查看规则，并使用责任人的仓库权限读取所填基线命中的文件原文；查看不会启用资源。预览限 30 个文件，每个文本最多展示 16000 字符；大文件、二进制和符号链接显示原因。拉取后才注入且未提交到远端的文件，下单前无法预览，页面会说明此限制。
