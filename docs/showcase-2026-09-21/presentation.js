...
function stop(){playing=false;clearTimeout(timer);clearParticles();$('#play').innerHTML='▶ <span>播放讲解</span>';$('#play').setAttribute('aria-label','播放完整讲解')}
function setScene(name,manual=true){if(manual){stop();beat=-1}scene=name;$('#diagram').inert=name==='philosophy';$('#diagram').setAttribute('aria-hidden',String(name==='philosophy'));$('#detail').hidden=true;$('#philosophy').hidden=name!=='philosophy';document.querySelectorAll('[data-scene]').forEach(b=>b.setAttribute('aria-selected',String(b.dataset.scene===name)));const ids=name==='delivery'?delivery:name==='knowledge'?[...knowledge,'design','deliver']:[];focus(ids);const s=scenes[name];caption(s.label,s.title,s.body);if(manual)$('#progress').style.width='12%'}
function showBeat(index){beat=(index+beats.length)%beats.length;const [name,ids,es,title,body]=beats[beat];setScene(name,false);focus(ids,es);caption(`${String(beat+1).padStart(2,'0')} / ${beats.length} · ${name==='knowledge'?'知识闭环':name==='delivery'?'交付闭环':name==='philosophy'?'设计思想':'全景架构'}`,title,body);$('#progress').style.width=`${(beat+1)/beats.length*100}%`;if(playing){clearTimeout(timer);timer=setTimeout(()=>{if(beat===beats.length-1)stop();else showBeat(beat+1)},7200)}}
...
function showDetail(n){stop();const id=n.dataset.nodeId,[category,body,foot]=details[id];$('#detail-category').textContent=category;$('#detail-title').textContent=graph.components.find(c=>c.id===id).label;$('#detail-body').textContent=body;$('#detail-foot').textContent=foot;$('#detail').hidden=false;focus([id]);$('#close-detail').focus()}
