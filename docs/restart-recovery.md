...
> 2026-09-12 更新：[执行连续性与独立部署](execution-continuity.md) 已支持。
> 配置独立 runtime 后，重启 Web 入口不执行下述任务回收；只有重启执行服务
> 才涉及恢复。Pi 现已持久化上下文，恢复原消息、工具结果和压缩摘要；
> 下面历史矩阵中的“无感”指流程可恢复，不代表运行中的命令从未中断。
