...
QUERIES += [
('callback-heldout','repo-a','对象析构后异步函数仍然访问它，如何防止悬空引用',['callback']),
('ut-heldout','repo-a','第一次编译 C++ 时单元测试依赖怎么准备',['ut']),
('symbol-heldout','repo-a','FMA 文件标识能不能拿文件显示名替代',['symbol']),
('module-heldout','repo-a','同一个网元反复上报相同事件，应使用什么字段判断重复',['module']),
('yaml-heldout','repo-a','YAML Consistence Check 失败要自己改吗',['yaml']),
('frontend-heldout','repo-a','Windows 工作台按钮看起来像普通文字怎么办',['noise']),
('json-heldout','repo-a','JSON 配置字段名称不对要检查什么',['json']),
('unrelated-heldout','repo-a','北京去上海的高铁票怎么买',[]),
('missing-heldout','repo-a','PostgreSQL WAL 归档恢复命令是什么',[]),
]
async def main():
 with tempfile.TemporaryDirectory(prefix='mae-knowledge-probe-') as tmp:
  ...
  for name,repo,q,expected in QUERIES:
   ...
   # Simulate host catalog selection, not answer-specific filtering.
   version={'v26':'old','v27':'new'}.get(name)
   eligible=[{'id':mid,'path':str(paths[key])} for mid,key in mapping.items()
      if any(k==key and status=='accepted' and (r==repo or scope=='platform' or (k=='module' and repo=='repo-a'))
             for k,r,scope,status,_,_ in DATA)
      and not (version and key in ('old','new') and key!=version)]
   hits=await side.search({'query':q,'repo':repo,'limit':5,'sources':eligible})
   row={'case':name,'query':q,'expected':expected,'raw': [{'key':mapping.get(m.memory_id_of(x['source'])),'score':round(x['score'],4)} for x in raw[:5]],'current_sidecar':[{'key':mapping[x['id']],'score':round(x['score'],4),'semantic_score':round(x.get('semantic_score',0),4)} for x in hits['hits']],'two_searches_ms':round((time.perf_counter()-start)*1000)}
   row['pass']=any(hit['key'] in expected for hit in row['current_sidecar']) if expected else not row['current_sidecar']
  ...
  for name in ['kernel-simplification','task-patrol']:
   p=docs/(name+'.md');p.write_text(f'---\nknowledge_id: doc:{name}\nasset_status: published\n---\n'+(ROOT/'skills'/name/'SKILL.md').read_text());await side.ingest({'path':str(p)})
  ...
  for q,want in [('重复审批和 SHA 检查让任务不断返工，如何简化流程','kernel-simplification.md'),('内网 PI 定时巡检生产任务能不能自动修复和推进','task-patrol.md')]:
   hits=(await side.search({'query':q,'repo':'repo-a','limit':5,'sources':[{'id':f'doc:{p.stem}','path':str(p)} for p in docs.glob('*.md')]}))['hits']
   result['real_documents'].append({'query':q,'expected':want,'hits':[{'file':Path(h['file']).name,'score':h['score']} for h in hits]})
  ...
  result['cases_passed']=sum(row['pass'] for row in result['queries'])
  result['cases_total']=len(result['queries'])
  result['positive_top1']=sum(bool(r['expected'] and r['current_sidecar'] and r['current_sidecar'][0]['key'] in r['expected']) for r in result['queries'])
  result['positive_total']=sum(bool(r['expected']) for r in result['queries'])
  args.output.parent.mkdir(parents=True,exist_ok=True)
  args.output.write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n')
  print('RESULT',json.dumps({k:v for k,v in result.items() if k!='queries'},ensure_ascii=False),flush=True)
  assert result['cases_passed']==result['cases_total'], '检索场景未全部通过，查看 JSON 的 pass 字段'
  assert all(r['hits'] and r['hits'][0]['file']==r['expected'] for r in result['real_documents']), '真实文档未首位命中'
  assert result['revision_new_text'] and not result['revision_old_text'] and not result['withdrawn_returned'] and not result['draft_indexed']
asyncio.run(main())
