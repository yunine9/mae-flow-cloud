...
编码与测试提示词已强化函数 UT、模块测试、合理隔离外部依赖，以及设计用例到测试代码和执行证据的对应关系。子任务 Spec 的“验收条件”已采用带稳定编号的条件／操作／预期一句话用例；当前任务责任人逐条处置意见，支持已修复、不采纳、延期和接受风险，理由与 Agent 原回执分别留存。已提交意见的作者修改或申请撤回不会替代责任人闭环，不提供批量闭环或管理员代签。Archify 独立展示仍属后续批次，详见 [实施设计](docs/module-delivery-redesign-2026-09-09.md)。
