...
- **2026-09-01 单仓拆分(交付单元)首版**(方案 docs/
  ...
  测试+双端 typecheck 绿。真 HTTP 外部账号点击 403、真实 GLM 拆三块
  到契约单元合入并自动解锁下一单元、单仓直干完整链及多仓并行起跑也已
  实测（根因总表「第五轮」）。**如实挂账**:①真实 GLM 五小时额度在
  第二单元编码和多仓并行子任务期间耗尽，剩余 MR 收口保留为可恢复现场，
  未伪报全链通过;②本轮三个单元均由 owner 负责，主责任人与单元责任人
  不同账号时的通知只做了服务层验证，未做真浏览器异账号通知复验;
  ③拆分不合理的返工走确认卡"需要修改"+批注循环,未单独建"重拆"入口。
  (codex 真
  ...
  不闭合、403 不点名主责任人、429 原始 JSON。契约单元 MR !1 与单仓直干
  回归 MR !3 已全链合入。
