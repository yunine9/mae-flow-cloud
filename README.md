...
- **2026-09-03 拆分变成分析的产物——下单的「大需求先分析再拆分」开关取消**
  ...
  需求/定规格读完仓、盘出改动面后判断一个 MR 装不下就调它;宿主受理即落盘
  `requirement_analysis_requested` 与 `split_escalation`,下一拍掐掉编码会话
  与容器(纪律同暂停:资源没确认释放就如实 failed),清空现场指针,按分析单
  重新排队——之后就是原有的分析→划分方向卡→方案→「确认并生成任务」→按单元
  建子任务,同仓单元仍按拓扑序补隐式串行边。分析提示词把上一位 Agent 的理由
  与切法当起点,明说"改动面要自己重新核实,切法可以推翻"。③拒绝口径一句话
  回给模型不抛错:子任务、多仓、已有推送/MR、非运行中、已受理过。④父单下单
  时填的单号不下传,确认卡逐单元填;旧编码现场作废不清理(与暂停同样留
  给工作区回收)。**已验**(`tests/splitProposal.test.ts`,宿主无内核):假编码
  会话被掐、现场指针清空、按分析单真跑只读克隆→剧本写图→确认卡两单元→
  确认后两个子任务串行;拒绝口径逐条。**未验**:真模型会不会在该拆时主动
  调这个工具(提示词只写了动作锚定的触发,没有硬闸);内核模式下编码会话
  已经跑到写代码阶段再提议时,旧克隆里的内核状态与容器被直接作废,没有跑过
  真容器的这条转身。
