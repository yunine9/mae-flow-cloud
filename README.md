...
- **2026-09-02 进度条阶段词表收进内核一份——五段,老任务不再各显各的**
  ...
  进度条的唯一来源;五段按用户模型:配置与需求 / 方案 / 开发 / 持续检视 /
  已合入——从最终检视、推送起直到合入都是「持续检视」("MR 创建只是微不
  足道的一步"),「已合入」独占 end 并新增 platform.merged 方案条目。宿主
  ...
  就当没有阶段轨道,由 `placeholderProgress` 按状态占一段(索引契约写在
  phases.json:0=受理未开工、1=大需求分析中、2=拆分后子任务开发中、末段=
  终态),**脉冲里的 step_id / 步骤标题 / 里程碑照常保留**——检视面与流程
