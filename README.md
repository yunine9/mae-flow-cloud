...
- **2026-09-06 停摆有类别了,但"先自愈再停"只在有预算上下文的入口成立。**
  ...
  (原来只在修复环存在时发,其余停摆手机上没动静)。**没做的**:6 个
  `catch` 点抓到内核异常仍是当场停(经分类器判"重放有意义"的只是记成基础
  设施类,没有进带预算的自愈)——那些入口拿不到恢复循环的 epoch,硬接会
  让检视/冲突派单的失败进交付恢复链,语义未验证。留待下一步单独收。
  逐点表见 docs/stall-sites-20260906.md。
