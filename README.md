...
- **2026-09-02 批注与检视改成右侧抽屉+筛选条+CodeHub 意见转批注**(用户
  ...
  ①**抽屉不是弹层**:宽屏(>1100px)挤进 `.ws-body` 栅格右栏顶替决策侧栏,
  左侧材料照常可点、可圈选新批注,"回到那一行"不再先关窗;窄屏退化成
  全屏覆盖,定位时才关抽屉(`matchMedia` 判断)。代价:抽屉开着时决策侧栏
  不可见,要拍板先关抽屉——原弹层也盖住它,没有变差。②**顶部筛选条**
...
- **2026-09-02 持续检视意见的排版——进度条下只报数,正文按来源竖排进
  ...
  ①进度条下换成 `FeedbackSummary`,一行按来源报"几条 / 几条进行中",带
  "在批注与检视里查看"按钮直接开弹层;②弹层里批注卡片之后追加两节
