...
- **2026-09-03 Build-Fix 证据勘误:平台笔记不算代码修改**(内网 task-31
  ...
  根因:`verifyPrePushEvidence` 把所有 Edit/Write 都当"最后一次代码修改",
  不分是不是能进交付的文件。现在 `.mae-flow-work/` 下的文件与 `.mae-flow*`
  状态文件(与 `.git/info/exclude` 登记的同一批,push 只传 HEAD,它们永远
  进不了交付)不计入;失效措辞点名是哪个文件、第几个事件让证据失效,人一眼
  能看出冤不冤;使命明说"写平台笔记不用重跑"。**口径不变的部分**:编译与
  UT 仍须在最后一次代码修改之后真实成功——这道闸不是为了防产物,是防"改完
  没重跑"的旧绿灯推上去烧慢流水线;push 前清单审核看的是文件范围,不验证
  构建,替代不了它。**已验**:`tests/prepushAgent.test.ts` 笔记/状态文件写在
  编译后不算改动、代码写在编译后仍判失效且措辞点名文件。**未验**:真 Build-Fix
  会话在内网的一轮实走;`openspec/config.yaml` 之类内核配置写入仍按代码
  修改算(它在 exclude 名单里但改它可能影响构建,宁严勿松)。
