...
- **2026-09-02 持续检视宿主能力链的死锁排查与修复**(用户拍板:"重点看
  agent 卡死、内核卡死、cloud 和内核摩擦;内核过于严格完全可以放松,
  目标是完成任务,不是跟看犯人一样管 agent"):实测复现并修掉六条
  **一次失败=永久失败、无命令可救**的路。①宿主权威收据原来封整份
  `delivery_loop` 与逐条意见正文,写盘不限体积、读回限 32 KiB——一轮
  12 条 350 字的 MR 检视(内核自己允许单条 4000 字)就越线,此后反馈、
  流水线登记乃至 **MR 合入后的 close** 全部报"收据过大",而制造死锁的
  那条命令自己报成功;现改封摘要,体积与反馈数量彻底解耦(收据里逐字
  存的 payload 同样只留摘要)。②凭据路径拿未解引用的 abspath 与
  realpath 化的信任根比,`<data>` 经过任何一层软链(macOS 的 `/var`、
  容器挂载、盘迁移)就一条宿主命令都过不去。③三个收据扫描函数对每份
  历史收据都走严格读取,一份权限被动过/写坏/超限就 SystemExit 掀掉整条
  命令(原来的 `except SystemExit: raise` 看着像跳过坏件,实际是空操作)
  ——现在**历史台账体检 fail-soft、鉴权仍旧 fail-closed**。④Cloud 从不写
  内核强制要求的宿主任务绑定,且 `pipeline record` / `intervention
  reconcile` 裸调 CLI 不带凭据;收编新内核当天全线报"无法读取宿主任务
  绑定"/"必须携带 Cloud 宿主凭据"。⑤"打开反馈前必须先有前驱收据"把
  首条宿主动作自己挡在门外(老任务升级、迁移现场一份收据都没有),
  现改为**有链才查链**;`close` 同理:PASS 登记在能力链之前的单子不再
  因为拿不出流水线收据而永远关不掉。⑥收据只按任务号哈希归属,同一
  任务号在另一份代码仓里会读到别人的陈账,现按"任务号+realpath(工作
  区)"归属。另外 Cloud 判断"这条宿主命令要不要带凭据"改为读**任务自己
  的执行契约**(与内核同一份事实),不再只看部署开关——错配会让流水线
  登记被内核拒收、任务永远停在"等待流水线证据核销"(实测 delivery E2E
  卡死 60 秒)。
  **已验**:真内核+真 git+真 RSA 的端到端复现与回归(量大检视整轮走通
  并能在合入后收口、软链信任根下可用、绑定由宿主写在工作区之外、
  流水线登记无凭据被拒/摘要不符被拒/配套凭据放行)、Cloud 全量、
  双 TypeScript、Web 生产构建、内核全量自测与基线逐一对拍。
  **未验**:真模型真容器下的长命 MR(几十轮反馈)累积表现;收据至今
  **不做清理**,长期只增不减,扫描成本随任务寿命线性增长——目前每份
  收据恒定几百字节,尚未成为问题,但没有实测过上千份时的耗时。
  **勘误**:上一条记录写的内核分支 `cloud/continuous-review` 已不准确,
  现行来源分支是 `cloud/workflow-customization`(见 `kernel/VENDORED`)。
