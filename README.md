...
- **2026-08-27 推送前编译的人工控制三件套(内网实锤驱动)**:部署重启
  ...
  **重跑**(POST /tasks/:id/prepush/retry,真在跑时服务端拒绝并明说
  "正在进行"——这句拒绝兼作活性探针)、**停止**(POST /tasks/:id/
  prepush/stop,停止≠放行,本轮如实收口成失败停机后跳过/重跑才亮;
  中止对竞态反复补刀直到收口,attempt 预算兜底不会无限等)、跳过
  (同上)。另:编译槽位排队真相写进 prepush.message(原来只在任务
...
- **知识块按触发词注入(2026-08-16)**:交付仓的
  ...
  被写成大杂烩,要进场用起来才知道;
- **业务知识与 Skill 消费足迹(2026-08-23)**:发起任务或确认跨仓方案时，
  可以按仓读取目录并选择 `docs/**/*.md(x)` 作为本单重点知识；根目录
  `AGENTS.md`/`AGENTS.override.md`/`CLAUDE.md` 仍按 Pi 规则自动加载。
  Cloud 记录主 Agent、子 Agent、开发助手和推送前验证在什么内核阶段
  加载、Read 或检索了哪篇文档/Skill，任务详情「执行现场」可回看。
  这份足迹只用于发现知识缺口、低利用材料和高价值 Skill，不参与步骤、
  审批或完成裁决；目录漂移、文档损坏、足迹写入失败都只告警不挡任务;
