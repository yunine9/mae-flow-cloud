...
- **2026-09-01 单仓拆分(交付单元)首版**(方案 docs/
  ...
  测试+双端 typecheck 绿。**如实挂账**:①scope-decision 的 HTTP 鉴权
  ("只有主任务责任人可裁决")是 server 路由一行 canOperate 判定,
  测试未起真 HTTP 服务;②真模型演练"单仓拆三块"待用户环境实跑;
  ③子任务列表默认折叠等前端视觉优化留给专门交互轮;④拆分不合理的
  返工走确认卡"需要修改"+批注循环,未单独建"重拆"入口。(codex 真
