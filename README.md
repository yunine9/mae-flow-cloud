...
- **2026-09-07 需求修订行漂移与跨段误拒收**：页面已有 Markdown 重锚定，
  但修订使命仍报旧行号，范围校验又只认起始行并直接匹配 Markdown 原文。
  现在两处复用重锚定，跨行范围同步平移并包含末行；原批注历史不覆盖。
  页面点击跳转时刷新原文与锚点检查，起止行、行内标记同步使用新位置；
  gone/ambiguous 不再提示后仍滚到旧行号，重复文字也不以旧行号猜命中。
  定位限制在当前材料及对应 diff 文件，避免命中另一个文件的同名行号。
  已验：跨段选区、加粗/图片结论段前插入内容、两批排队间的行漂移，以及
  仍拒绝越界修改。锚点消失、重复文字或选区内部发生结构变化时，不能仅
  靠行号推定准确位置，仍需结合当轮副本核对；未取得本次内网失败副本。
