...
import { renderPlantUml } from "../src/plantumlRender.ts";
...
test("架构视图真实浏览器：迟到旧图不能覆盖新版，读取失败卸载旧图，空态可回 Story", {
  ...
  try {
    ...
    const uml = await renderPlantUml("@startuml\ntitle 订单类关系\ninterface OrderStore {\n  + read()\n  + transact(merge)\n}\nclass OrderSync {\n  + syncOrders()\n}\nOrderSync --> OrderStore : 合并写入\n@enduml");
    assert.ok(uml.svg, uml.error); assert.ok(!uml.syntax_error);
    const result = await build({ entryPoints: [resolve("tests/browser/storyArchitecture.tsx")], bundle: true, write: false,
      ...
      define: { "process.env.NODE_ENV": '"production"', PLANTUML_SVG: JSON.stringify(uml.svg), ARCHIFY_HTML_ONE: JSON.stringify(html[0]), ARCHIFY_HTML_TWO: JSON.stringify(html[1]) } });
    ...
    assert.deepEqual(JSON.parse(outcome), { umlNavigable: true, umlFullscreen: true, raceProtected: true, staleRemoved: true, failureReadable: true, emptyReadable: true, opened: true });
