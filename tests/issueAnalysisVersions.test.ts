 ...
 * - 推导:第 j 份快照 = 第 j 批意见提交时的版本,live 恒为最新版;
 *   命名 v1=初版、v_i=修订(i-1);每版带该批提交的意见 id;
 * - 去重:快照=live(AI 未修订完)与相邻同文快照都不重复出条;
 * - 非检视通道的报告重写不产生冻结版本(没检视过永远只有 live 一版);
...
test("去重:快照=live(AI 未修订完)与相邻同文快照都不重复出条", async () => {
  ...
  addReview(root, { author: "dev", line: 3, anchor: "根因一", note: "修一" });
  submitReviews(root);
  // live === s1:提交时冻结的就是当前稿,修订版还不存在 → 只有初版。
  ...
  await tick();
  // AI 还没修订就攒了第二批并提交:新快照与上一份同文 → 折并,不出假版本。
