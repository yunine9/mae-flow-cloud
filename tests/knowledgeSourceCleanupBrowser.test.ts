...
test("领域清理桌面交互：按仓可选清理、手动开始不校验 MR、范围修改需重新预览", { skip: !existsSync(chrome) && "需要 Chrome" }, async () => {
  const root = mkdtempSync(join(tmpdir(), "source-cleanup-browser-"));
  try {
    const bundle = await build({ entryPoints: [resolve("tests/browser/domainSourceCleanup.tsx")], bundle: true, write: false, format: "iife", jsx: "automatic", loader: { ".css": "empty" }, jsxImportSource: resolve("web/node_modules/react"), define: { "process.env.NODE_ENV": '"production"' } });
