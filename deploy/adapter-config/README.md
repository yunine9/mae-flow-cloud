...
入口为具体任务工作台的「任务详情 → AR 单号 → 纠正单号」，填写新单号和
AR 准确描述。由责任人或管理员发起，宿主程序执行，无需 Agent 参与。
仅修改单号、任务分支、任务提交前缀和交付引用；代码、未提交内容、已有
验证和检视结论保留。已合入任务不改历史，兄弟任务的单号不会连带修改。

未推送时只改本地。已推送时先建立新远端分支；已有 MR 时先创建关联新
单号的新 MR，再关闭旧 MR、删除旧远端分支。新分支只迁移原先已发布的
代码，不捎带本地尚未发布的提交。中断或失败在同一入口显示原因和重试；
新交付已经生效但旧资源清理失败时，清理不阻塞新交付。

升级需要一并应用本目录补丁并重启 adapter 和 serve，新增 `mr_close`
端点使用已有 CodeHub REST 地址，PUT `state_event=close`，只有平台明确
返回 `closed` 才继续删除旧分支。执行凭据需要关闭该 MR 的权限。
未配置或平台拒绝关闭时保留旧 MR 与旧分支，在任务详情显示清理待重试。
本地已验证真实 Git、内核与模拟平台响应；内网 CodeHub 的关闭端点还需用
专用测试 MR 验收，不要关闭真实业务 MR 做接口测试。
