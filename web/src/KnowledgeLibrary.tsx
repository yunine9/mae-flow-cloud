...
export function KnowledgeLibrary({ category, onCategoryChange, uploadRequest, onOpenTask, onManage }: {
  ...
  useEffect(() => { const sync = () => { const next = routePage(); setPage(next); setVisited(p => new Set([...p, next])); setComponent(new URLSearchParams(location.search).get("componentResearch") ?? ""); setDomain(new URLSearchParams(location.search).get("domainExtraction") ?? ""); }; addEventListener("popstate", sync); return () => removeEventListener("popstate", sync); }, []);
  ...
  return <section className="space-y-5"><nav className="tw-root flex gap-2 border-b border-line pb-4" aria-label="知识库子页面">{([["documents", "知识文档"], ["component", "基础组件萃取"], ["domain", "领域知识萃取"]] as const).map(([value, label]) => <Button key={value} variant={page === value ? "default" : "outline"} aria-pressed={page === value} onClick={() => choose(value)}>{label}</Button>)}{page === "documents" && <Button className="ml-auto" onClick={() => setUpload(n => n + 1)}>＋ {category === "skills" ? "添加 Skill" : "添加知识"}</Button>}</nav>
