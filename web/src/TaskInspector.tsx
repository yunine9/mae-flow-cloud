      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-6">
        {kind === "details" && <div>
          ...
          <dl className="m-0 mb-5 grid grid-cols-3 gap-x-6 gap-y-4">
            <div><dt className={dtc}>负责人</dt><dd className="mt-1 text-sm leading-[1.7] [overflow-wrap:anywhere]"><PersonName account={task.luban_account} /></dd></div>
            <div><dt className={dtc}>任务状态</dt><dd className="mt-1 text-sm leading-[1.7] [overflow-wrap:anywhere]">{statusText(task)}</dd></div>
            <div><dt className={dtc}>任务编号</dt><dd className="mt-1 text-sm leading-[1.7] [overflow-wrap:anywhere]"><code>{task.id}</code></dd></div>
            <div><dt className={dtc}>创建时间</dt><dd className="mt-1 text-sm leading-[1.7] [overflow-wrap:anywhere]">{formatLocalDateTime(task.created_at, { year: true })}</dd></div>
            {task.updated_at && <div><dt className={dtc}>最近更新</dt><dd className="mt-1 text-sm leading-[1.7] [overflow-wrap:anywhere]">{formatLocalDateTime(task.updated_at, { year: true })}</dd></div>}
            {task.completed_at && <div><dt className={dtc}>完成时间</dt><dd className="mt-1 text-sm leading-[1.7] [overflow-wrap:anywhere]">{formatLocalDateTime(task.completed_at, { year: true })}</dd></div>}
          </dl>
          <section className="border-t border-line py-4"><h3 className="m-0 mb-3 text-sm">进度与交付</h3>
            <dl className="m-0 grid gap-3.5">
              <div className="grid grid-cols-[100px_minmax(0,1fr)] gap-x-[18px]"><dt className={cn(dtc, "pt-[3px]")}>当前步骤</dt><dd className="m-0 text-sm leading-[1.7] [overflow-wrap:anywhere]">{task.progress?.step || task.detail || "暂无步骤记录"}</dd></div>
              {task.progress?.milestone && <div className="grid grid-cols-[100px_minmax(0,1fr)] gap-x-[18px]"><dt className={cn(dtc, "pt-[3px]")}>子任务里程碑</dt><dd className="m-0 text-sm leading-[1.7] [overflow-wrap:anywhere]">{task.progress.milestone.title} · {task.progress.milestone.event}
                {task.progress.milestone.reason && ` · ${task.progress.milestone.reason}`}</dd></div>}
              <div className="grid grid-cols-[100px_minmax(0,1fr)] gap-x-[18px]"><dt className={cn(dtc, "pt-[3px]")}>合入请求</dt><dd className="m-0 text-sm leading-[1.7] [overflow-wrap:anywhere]">{task.delivery?.mr_url ? <a href={task.delivery.mr_url} target="_blank" rel="noreferrer">
                打开合入请求 · {task.delivery.mr_state || "查看状态"} ↗</a> : "尚未记录合入请求"}</dd></div>
              {task.delivery?.pipeline && <div className="grid grid-cols-[100px_minmax(0,1fr)] gap-x-[18px]"><dt className={cn(dtc, "pt-[3px]")}>流水线</dt><dd className="m-0 text-sm leading-[1.7] [overflow-wrap:anywhere]">{task.delivery.pipeline}</dd></div>}
          ...
          <nav className="border-t border-line" aria-label="任务执行信息">
            <button type="button"
              className="flex w-full cursor-pointer items-center justify-between gap-5 border-0 border-b border-line bg-none py-3 text-left text-text hover:bg-surface-2 hover:text-ink focus-visible:outline-2 focus-visible:outline-offset-[3px] focus-visible:outline-primary"
              onClick={() => onInspect("usage")}><span><strong className="text-[13px] font-semibold">模型用量</strong>
            ...
            <button type="button"
              className="flex w-full cursor-pointer items-center justify-between gap-5 border-0 border-b border-line bg-none py-3 text-left text-text hover:bg-surface-2 hover:text-ink focus-visible:outline-2 focus-visible:outline-offset-[3px] focus-visible:outline-primary"
              onClick={() => onInspect("workflow")}><span><strong className="text-[13px] font-semibold">执行方案</strong>
        ...
        {kind === "timing" && <><p className="m-0 mb-5 text-[13px] leading-[1.7] text-muted-foreground">按已记录的事件估算历史耗时；任务当前状态以工作台为准。</p><TaskTimeline taskId={task.id} defaultOpen /></>}
        {kind === "usage" && (task.token_usage ? <TokenUsage usage={task.token_usage} placement="detail" />
          : <p className="m-0 mb-5 text-[13px] leading-[1.7] text-muted-foreground">模型提供方暂未返回用量，不能据此认为消耗为零。</p>)}
        {kind === "workflow" && <>
          {task.workflow_profile ? <WorkflowProfileCard profile={task.workflow_profile} warning={task.workflow_profile_warning} />
            : <p className="m-0 mb-5 text-[13px] leading-[1.7] text-muted-foreground">本任务没有记录执行方案。</p>}
          {(task.execution_plan_alerts ?? []).map((line, index) => <p className="m-0 mb-5 text-[13px] leading-[1.7] text-muted-foreground" key={index}>{line}</p>)}
        </>}
      </div>
    </SheetContent>
  </Sheet>;
