...
export function WarmupBadge({ task, onOpen }: { task: TaskSummary; onOpen: () => void }) {
  ...
  const descriptions = {
    ...
    passed: "基线编译已通过，构建缓存已就绪；本次修改的验证结果另见推送前验证。",
    failed: "基线编译失败，点击查看环境或上游问题。",
    infrastructure_failure: "基础设施问题导致准备未完成，点击查看原因。",
    unknown: "尚未收到基线编译记录，暂时无法确认是否就绪。",
  ...
  return <button type="button" className={`warmup-badge is-${state}`} aria-haspopup="dialog"
    ...
    <i aria-hidden /><span>基线编译</span><b>{labels[state]}</b><span aria-hidden>↗</span>
  </button>;
}

export function WarmupPanel({ task }: { task: TaskSummary }) {
  const receipt = task.baseline_build;
  if (!receipt) return <p className="warmup-empty" role="status">尚未收到基线编译记录，暂时无法确认是否就绪。收到检查结果后，顶部状态会自动更新。</p>;
