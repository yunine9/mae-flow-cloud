 ...
 * parseWorkspaceImagePath 同尺,两边谁也放不进对方拒的。
 */

const TICKET_FORM = /^ticket-images\/([^/]+)\/[0-9a-f]{16}\.[a-z]+$/i;
const REGISTRATION_FORM = /^issue-images\/[0-9a-f]{16}\.[a-z]+$/i;
...
export function resolveWorkspaceImage(
  ...
  if (ticket) {
    if (ticket[1]?.includes("..")) return undefined;
