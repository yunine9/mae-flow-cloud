...
function skillKey(repository: string, relativePath: string): string {
...
export function RepositorySkillPicker({
  ...
  function wantedSkills(): Set<string> {
    if (!catalog) {
      return new Set(initialSkills.map((skill) =>
        skillKey(skill.repository, skill.relative_path)));
    }
    const wanted = new Set<string>();
    for (const repository of catalog.repositories) {
      for (const skill of repository.skills) {
        if (selectedIds.has(skill.id)) {
          wanted.add(skillKey(repository.repository, skill.relative_path));
        }
      }
    }
    return wanted;
  ...
  async function scanSkills() {
    ...
    setPickerState({
      catalog: null,
      scanError: "",
      selection: { scanned: false, scanning: true, selectedIds: [] },
    });
    try {
      ...
      const nextIds = result.repositories.flatMap((repository) =>
        repository.skills
          .filter((skill) => skill.selectable
            && preserve.has(skillKey(repository.repository, skill.relative_path)))
          .map((skill) => skill.id))
      ...
      setPickerState({
        ...
        scanError: reason instanceof Error
          ? reason.message
          : "仓内能力读取失败，请稍后重试",
        selection: { scanned: false, scanning: false, selectedIds: [] },
      });
    } finally {
  ...
  return (
    <section className={`${presentation === "launch" ? "launch-form-section " : ""}repository-skills-section repository-skills-${presentation}`}
      ...
      <div className="repository-skills-head">
        <i aria-hidden>＋</i>
        <div>
          <strong id={`repository-skills-title-${presentation}`}>仓内能力（可选）</strong>
          <small>{presentation === "decision"
            ? "按仓选择，随本次确认下发给对应交付子任务"
            : "让 Agent 复用各代码仓随仓维护的 Skill"}</small>
        </div>
      ...
      <div className="repository-skills-toolbar">
        <p>
          只读取各仓基线中 <code>.agents/skills</code>、<code>.pi/skills</code>、
          <code>.claude/skills</code>、<code>.cac/skills</code> 下一级的 Skill。
          选中后由 Agent 按任务内容自行判断何时使用，不会扩大仓库或工具权限。
        </p>
        <div>
          {catalog && (
            <small className={atLimit ? "at-limit" : ""}>
              已选 {selectedIds.size}/{MAX_SELECTED_SKILLS} 项
            </small>
          )}
      ...
      {!hasRepository && (
        <div className="repository-skills-empty">
          先填写代码仓，再按需读取仓内能力；不读取也可继续。
        </div>
      )}
      {!catalog && !scanError && initialNames.length > 0 && (
      ...
      {scanError && (
        <div className="repository-skills-error" role="alert">
          ...
          <span>{scanError}</span>
          <small>你可以重试，也可以不改 Skill 选择直接提交决定。</small>
        </div>
      )}
      {catalog && catalog.repositories.length === 0 && (
        <div className="repository-skills-empty">
          这些仓库没有可展示的 Skill，不影响继续提交。
        </div>
      )}
      {catalog && catalog.repositories.length > 0 && (
