...
{{blue|待接纳}}表示正在评估；{{blue|已接纳}}表示方向确认但未必已经上线；{{green|已闭环}}表示已经有可验证结果；{{red|未接纳}}必须附理由。评论和状态变化都会留在同一张卡片上。`,
  ...
  },
  {
    ...
    steps: [
      { title: "选择人工介入程度", detail: "需求交付与问题处理各一组四档设置，分别控制两条流之后新节点的停留方式，互不影响。" },
      { title: "配置代码身份", detail: "提交邮箱和 CodeHub 访问凭据用于拉代码、提交、创建合并申请和确认检视身份。" },
      { title: "配置小鲁班", detail: "保存个人通知凭据并测试送达；手机回复仍需先 /mfc 激活插件。" },
    ],
    body: `## 人工介入程度
设置页有两组一模一样的四档旋钮：「需求交付」管需求任务的确认频率，「问题处理」管问题单处理流的确认频率，两边独立取值、互不带动。每组四档其实只回答两个问题：任务做到中间要不要停下来问你，推送前要不要让你过目变更清单。无论选哪一档，流水线都必须检查当前这版代码，合并申请也仍然要由人来合并。“全自动”不会关掉这些基本检查。

过程有人（全程把关、逐步确认）时，问题分析开始前还会让你圈选仓内的排障知识（skill）：勾选的会成为 AI 的必读材料，一个都不选则 AI 自主决定。过程自动（只看交付、全自动）不圈选，AI 按既定次序自己取用。
