...
人工决定采用先到生效。若另一个页面、通知入口或协作者已经完成同一待办，旧页面不能覆盖新事实。刷新后会展示最新决定；尚未送达的补充意见会保留，允许按新的任务状态重新提交。`,
  ...
  },
  {
    ...
    steps: [
      ...
      { title: "填齐真实现场", detail: "手工登记常开网管环境四项：环境 IP、页面账号、页面密码和后台密码；页面账号默认 admin，可修改但不能为空。" },
...
## 工具与权限
拉日志、构建部署、读取 DTS 和代码仓，都通过平台提供的专用工具完成。浏览器草稿不保存网管口令，服务端会加密保存；口令不会出现在会话列表、状态摘要或事件流中，但为了让 AI 操作页面、抓日志和换库，会以明文进入{{red|当前问题的 AI 上下文}}。只填脱敏演示或现场专用口令，绝不要填写个人复用或生产口令。工具失败时，页面会显示真实原因，并告诉你还缺什么。`,
    screenshots: [{
