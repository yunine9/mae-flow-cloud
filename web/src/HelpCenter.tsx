...
{{blue|待接纳}}表示正在评估；{{blue|已接纳}}表示方向确认但未必已经上线；{{green|已闭环}}表示已经有可验证结果；{{red|未接纳}}必须附理由。评论和状态变化都会留在同一张卡片上。`,
  ...
  },
  {
    ...
    body: `## 人工介入程度
设置页有两组旋钮:「需求交付」是四档(过程问你/推送问你两个开关的组合),「问题处理」是三档——全自动、仅分析报告(默认)、全程把控。两边独立取值、互不带动。无论怎么选,流水线都必须检查当前这版代码,合并申请也仍然要由人来合并。"全自动"不会关掉这些基本检查。

问题处理三档的含义:**全自动**=AI 不再向你提问,环境信息不足就用已有信息分析并在报告里说明,一路跑到 MR 变绿(或办不了停下)才提醒你;**仅分析报告**=只在分析报告检视时停一次,你提意见(报告圈注或对话),AI 闭环重出直到你点确认,其余直达变绿;**全程把控**=现象对齐、结论确认、环境闸、推送过目全保留。任何档位下,流水线红灯需要人在交付平台处理/豁免的卡都照常等真人。
