...
import { Search, Layers, FileText, Download, ChevronRight } from "lucide-react";
...
export function ComponentResearchReview({ record, onChanged }: {
  ...
  useEffect(() => { reader.current?.scrollTo({ top: 0 }); }, [record.id, section?.id, view]);
  function scrollToSection(id: string) {
    const target = document.getElementById(`research-section-${id}`);
    const panel = reader.current;
    if (target && panel) panel.scrollTo({ top: panel.scrollTop + target.getBoundingClientRect().top - panel.getBoundingClientRect().top - 20, behavior: "smooth" });
  }
  ...
  return <section aria-label="组件审核工作区" className="research-review">
    ...
    {view === "document" ? <div ref={reader} tabIndex={0} aria-label="完整文档阅读区" className="research-reader research-full-document"><p className="mb-4 text-sm text-muted-foreground">完整研究草稿（含未勾选项）；下载与采纳仅包含勾选项，跨仓概述保留。</p>
      <Markdown text={`# ${record.topic}\n\n${record.document!.overview || "正在联合研究，章节将逐步保存…"}`} />
      <nav aria-label="文档组件目录" className="my-4 flex flex-wrap gap-2">{sections.map(item => <Button key={item.id} variant="outline" size="sm" onClick={() => scrollToSection(item.id)}>{item.title}</Button>)}</nav>
      ...
      : <div className="research-review-panes">
        <aside className="research-capabilities" aria-label="组件能力目录">
          <div className="research-capabilities-header"><strong>组件能力 <span>{sections.length}</span></strong>
          <div className="research-capability-search"><Search size={16} />
          <Input aria-label="筛选组件能力" placeholder="搜索能力名称…" value={query} onChange={e => setQuery(e.target.value)} /></div>
          ...
          <div className="research-capability-list" tabIndex={0} aria-label="组件能力列表">
            {sections.filter(s => s.title.toLocaleLowerCase().includes(query.toLocaleLowerCase())).map(item => <div key={item.id} className={`research-capability${section?.id === item.id ? " is-current" : ""}`}>
              <input type="checkbox" className="research-capability-check" aria-label={`纳入 ${item.title}`} checked={item.selected} disabled={busy || active || readonly}
              ...
              <button className="min-w-0 flex-1 text-left" onClick={() => { setSelected(item.id); setMessage(""); setError(""); }} aria-pressed={section?.id === item.id}>
                <span className="research-capability-title">{item.title}</span>
              ...
              </button><ChevronRight size={15} className="research-capability-arrow" />
            </div>)}
            {!!sections.length && !sections.some(s => s.title.toLocaleLowerCase().includes(query.toLocaleLowerCase())) && <p className="p-4 text-sm text-muted-foreground">没有匹配的组件能力</p>}
            {!sections.length && <p className="py-5 text-sm text-muted-foreground">正在梳理跨仓关系与细粒度能力，发现的组件会列在这里，默认全选。</p>}
          </div>
        </aside>
        <div ref={reader} tabIndex={0} aria-label="组件详细文档" className="research-reader">
          {section ? <>
