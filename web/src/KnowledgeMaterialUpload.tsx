...
export function KnowledgeMaterialUpload({ materials, onChange, onBusy }: { materials: MaterialSummary[]; onChange: (materials: MaterialSummary[]) => void; onBusy?: (busy: boolean) => void }) {
  ...
  return <section className="space-y-3 rounded-lg border border-line p-4" aria-label="业务资料上传">
    ...
    <div className="flex items-center gap-3"><Button type="button" variant="outline" disabled={busy} onClick={() => fileInput.current?.click()}>{busy ? "上传中…" : "选择文件"}</Button><span className="text-sm text-muted-foreground">可多选文档或 ZIP，选择后自动上传</span></div>
    <input ref={fileInput} hidden aria-label="上传业务资料" type="file" multiple disabled={busy} accept=".md,.txt,.docx,.pdf,.xlsx,.pptx,.zip" onChange={e => { const files = Array.from(e.target.files ?? []); if (files.length) void upload(files); e.target.value = ""; }} />
