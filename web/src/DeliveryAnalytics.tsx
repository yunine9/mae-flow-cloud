...
function Rework({ counts }: { counts: OriginCounts }) {
  const total = sumOrigins(counts);
  return <><div className="delivery-rework-total"><strong>{num(total)}</strong><span>增删行次</span></div><div className="delivery-stacked">{categories.filter(c => c.key !== "first").map(c => <span key={c.key} title={`${c.label}：${num(counts[c.key])}`} style={{ background: c.color, width: `${total ? counts[c.key] / total * 100 : 0}%` }} />)}</div><div className="delivery-rework-legend">{categories.filter(c => c.key !== "first").map(c => <span key={c.key}><i style={{ background: c.color }} />{c.label}<b>{num(counts[c.key])}</b></span>)}</div></>;
}
function TaskMetricRow({ id, title, tasks, child, onSelect }: { id: string; title: string; tasks: DeliveryAnalysisRow[]; child?: boolean; onSelect: (id: string) => void }) {
  const values = aggregateDelivery(tasks);
  return <tr className={child ? "delivery-child-row" : undefined}><td><button className="delivery-task-link" onClick={() => onSelect(id)}><b>{id}</b><span>{title}</span></button></td><td>{tasks.every(t => t.merged) ? "已合入" : "进行中 · 暂计"}</td><td>{values.available ? `${num(values.total)} 行` : "—"}</td><td className="delivery-accent">{percent(values.firstPercent)}</td><td>{values.available ? `${num(sumOrigins(values.rework))} 行次` : "—"}</td><td>{values.available}/{tasks.length}{!values.available && <small>待取证</small>}</td></tr>;
}
export function DeliveryAnalytics() {
  ...
  return <div className="delivery-analysis">
    ...
    <div className="delivery-scope"><BarChart3 size={18} /><span>团队汇总仅计已合入交付 · {summary.available} / {merged.length} 个交付任务有统计证据 · 父任务按子任务汇总，不重复计数</span></div>
    <div className="delivery-top-grid"><section className="delivery-card"><h2>最终代码来源 <span>{num(summary.total)} 行</span></h2><Donut counts={summary.retained} /><p className="delivery-muted">以最终交付差异中的新增/修改行为分母，追溯最后修改它的提交。最终差异删除行另计 {num(summary.deleted)} 行。</p></section><section className="delivery-card"><h2>累计返工量</h2><Rework counts={summary.rework} /></section></div>
    <section className="delivery-card"><h2>任务明细 <span>点击查看来源、返工与提交记录</span></h2><div className="delivery-table-scroll"><table><thead><tr><th>需求 / 任务</th><th>状态</th><th>最终代码</th><th>首次保留</th><th>累计返工</th><th>统计覆盖</th></tr></thead><tbody>{groupRows.slice(currentPage * 10, currentPage * 10 + 10).map(([id, tasks]) => {
      const parent = tasks.some(t => t.parent_id === id);
      return <Fragment key={id}><TaskMetricRow id={id} title={parent ? `${tasks[0].parent_title ?? "跨任务交付"} · ${tasks.length} 个子任务` : tasks[0].title} tasks={tasks} onSelect={setSelected} />{parent && tasks.map(row => <TaskMetricRow key={row.id} id={row.id} title={row.title} tasks={[row]} onSelect={setSelected} child />)}</Fragment>;
    ...
    <details className="delivery-method"><summary>统计口径与数据边界</summary><p>首次提交是任务基线之后的第一个非合并提交，不是第一次推送。仅统计源码、测试及脚本；文档、配置、锁文件、常见生成目录和二进制不进入代码分母。首次提交只含文档时，保留代码可以为零。</p><p>流水线 / 检视归因来自宿主实际修复轮次及前后提交范围。混合修改、外来提交和缺失证据归“其他”；不会分析提交标题猜测原因。历史改写或缺少合入前快照时，该任务暂不纳入汇总。合并提交不计入返工量（避免把上游改动当作返工），其中的冲突解决目前不计入行次；纯重命名不算返工，修改一行按删除旧行 + 新增新行计 2 行次。</p><p>这不是代码质量评分；不同任务规模与范围不同。覆盖不足时请结合任务明细解读趋势。采集在推送后后台进行，稍后刷新可查看结果，不阻塞开发交付。</p></details>
    <Sheet open={selected !== null} onOpenChange={open => { if (!open) setSelected(null); }}><SheetContent initialFocus={detailTitle} className="delivery-detail-sheet bg-(--surface) gap-0 data-[side=right]:w-[min(760px,58vw)] data-[side=right]:sm:max-w-[min(760px,58vw)]"><SheetHeader><SheetTitle ref={detailTitle} tabIndex={-1}>{selected} · 交付分析</SheetTitle><SheetDescription>{detailRows.length === 1 ? detailRows[0].title : `按 ${detailRows.length} 个子任务汇总`}</SheetDescription></SheetHeader><div className="delivery-detail-body">{!detailRows.length ? <p>未找到该任务的交付记录。</p> : <><p className="delivery-scope">{detailRows.every(r => r.merged) ? "已合入" : "进行中，以下为最近推送版本的暂计数据"} · 证据覆盖 {detail.available}/{detail.tasks}</p><section className="delivery-card"><h2>最终代码来源</h2><Donut counts={detail.retained} /></section><section className="delivery-card"><h2>累计返工</h2><Rework counts={detail.rework} /></section>{detailRows.map(row => <section className="delivery-card" key={row.id}><div className="delivery-detail-heading"><h2>{row.id} · 提交记录</h2><a href={`/work/${encodeURIComponent(row.id)}`} target="_blank" rel="noreferrer">工作台 <ExternalLink size={14} /></a>{row.mr_url && <a href={row.mr_url} target="_blank" rel="noreferrer">MR <ExternalLink size={14} /></a>}</div>{row.unavailable && <p className="delivery-note">{row.unavailable}</p>}{row.metric && <><p className="delivery-muted">比较 {row.metric.base.slice(0, 8)} → {row.metric.head.slice(0, 8)} · 排除 {row.metric.excluded_files} 个非代码文本文件</p><ol className="delivery-timeline">{row.metric.commits.map(c => <li key={c.sha}><i style={{ background: categories.find(k => k.key === c.origin)?.color }} /><div><strong>{categories.find(k => k.key === c.origin)?.label}</strong><span className="delivery-commit-size">+{num(c.additions)} / −{num(c.deletions)}</span><p>{c.subject}</p><small>{c.sha.slice(0, 10)} · {new Date(c.at).toLocaleString("zh-CN")}</small></div></li>)}</ol></>}</section>)}</>}</div></SheetContent></Sheet>
  </div>;
}
