...
3. Story 的 2.1.2 性能规格只写可量化性能指标；2.2.2 只写 REST、CORBA、RPC、消息、文件协议等对外或跨组件接口。
   ...
   测试设计当前只落实函数级 UT 和模块级 UT；3.2 保留为后续能力，不自动加入单独 REST 接口调试、跨模块集成、MST 或部署后验证，不把这些预留能力记为本轮待执行或未完成项。业务接口设计仍须完整。
...
Story 转测自检表属于后续转测阶段：本轮适用的串讲、翻译、DT、自验证等动作未执行时如实保留“待确认”“待执行”，不得为进入编码改成“已完成”；不在本轮范围的预留能力写明“不适用”及原因，不制造待执行义务。设计正文与实施附录的真实未决项仍须在确认前呈现并裁决；不要把设计未决项移入自检表规避检查。
