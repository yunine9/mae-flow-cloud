...
## 2026-09-04：构建现场不再逼用户决定 `.gitignore`

修复 `manifest --unchanged` 把已证明的编译产物当成待交付改动、继而诱导
Agent 修改业务仓 `.gitignore` 并向用户出确认卡的问题。现在只将两类文件
认作本地构建现场：形态明确的产物，以及有 COMPILE 副作用凭据且不是源码的
文件；Agent 直接编辑过的文件永远不会被自动排除。现场按路径和内容指纹写入
空交付清单，之后内容变化仍会被 `done` 拦住，避免把真实改动悄悄漏掉。

同步撤销“为本轮产物补 `.gitignore`”的流程指引。构建现场留在本地即可，
只有需求明确包含仓库治理时，仓库级忽略规则才属于本单交付内容。
