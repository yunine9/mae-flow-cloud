...
2. 执行 `python "{MAEFLOW_PATH}" role-task story-generate`，把输出的唯一启动话术原样交给 story-generator-agent。任务卡明确给出原始用户输入阅读副本（如有）、`decisions.md`、`.mae-flow-work/{单号}/spec.md`、`grill.md`、相关 `docs/specs/*.md`、两个模板和代码路径。
...
已有检视同时核对决定一致性：按原始答复的时间和语境判断哪些旧决定被取代，逐项对照受影响
BEH/TC、Story 与实施附录，点名两边冲突的原句和来源。不能只看决定编号都在就报 CLEAR。
发现明确矛盾由主 Agent 原地修订相关文档；生成者不能越过写入边界偷偷改 Spec，须将需要
同步的具体条目交回主 Agent。没有矛盾不为摘要/文件时间变化重启检视，不加审批步骤。
